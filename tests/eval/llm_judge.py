...
import pandas as pd
from src.text_generation.models import (GPT4, Claude3Point5Sonnet, Gemini1Point5Pro)
from src.text_generation import text_generation as tg
from src.data_directories import results_dir, prompts_dir
import json, re

if "tests/eval" in os.getcwd():
...
def prepare_data(is_sar = True):
    if is_sar:
        results_file_path = results_dir + "results-combined_prompts_SAR/recommended_cities_sar_gemma.csv"
    else:
        results_file_path = results_dir + "results-combined_prompts/recommended_cities_gemma.csv"
...
def judge(model_name: str, file_to_save: str, is_sar: bool = True):
    model = MODELS[model_name]
    data = prepare_data(is_sar)
    # data = data[:1]
...
def main(is_sar= True):
    # model_name = "Claude3Point5Sonnet"
    # model_name = "GPT-4"
    model_name = "Gemini1.5Pro"
    if is_sar:
        file_to_save = results_dir + f"results-combined_prompts_SAR/llm-judge/judged_cities_{model_name}_v2.csv"
    else:
        file_to_save = results_dir + f"results-combined_prompts/llm-judge/judged_cities_{model_name}_v2.csv"
